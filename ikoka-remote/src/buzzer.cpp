 ...
 *
 * Ikoka Nano: the J4 piezo has a pin per leg (D18/D19) instead of one buzzer
 * pin to ground. Drive is still single-ended -- tone() on BUZZER_PIN, the other
 * leg parked LOW as the return path -- so every pattern below is unchanged.
 */
...
// Resolved through g_ADigitalPinMap so it follows BUZZER_PIN rather than a
// hard-coded port/bit -- the piezo sits on P1.05/P1.03 here, not P1.12.
...
void BuzzerController::begin() {
  ...
  // Piezo return leg: parked LOW for the life of the program, so tone() on
  // BUZZER_PIN swings the piezo against a hard ground.
  pinMode(BUZZER_GND_PIN, OUTPUT);
  digitalWrite(BUZZER_GND_PIN, LOW);
  setHighDrive(BUZZER_PIN);
  setHighDrive(BUZZER_GND_PIN);
  noTone(BUZZER_PIN);
}
