...
void enterDeepSleep() {
  ...
  // On this board the reset line is also the E22's 5 V boost enable, so taking
  // it low powers the module down completely instead of leaving it in its own
  // sleep mode. nRF52 System OFF retains GPIO state, so it stays down. Wake is
  // a full reset through setup(), which brings the rail back up.
...
// ─── Remote battery (XIAO nRF52840 Plus) ─────────────────────────────────────
// Same charger hardware as the plain XIAO, and the same BSP pin numbers; only
// PIN_VBAT's index moves (32 on the plain variant, 35 here), which is why it is
// used symbolically. Port assignments below are from the Plus variant.cpp.
// PIN_VBAT  = 35 = P0.31  battery sense (1MΩ:510kΩ divider output)
// pin 14    = P0.14       enable divider, active LOW
// pin 23    = P0.17       charge state (~CHG), LOW = charging
// pin 22    = P0.13       charge current (HICHG), LOW=100mA HIGH=50mA
#define VBAT_ENABLE_PIN  14   // P0.14, OUTPUT LOW to connect divider
#define CHG_STATE_PIN    23   // P0.17, INPUT, LOW = charging
#define CHG_CURR_PIN     22   // P0.13, OUTPUT, LOW=100mA
...
void setup() {
  Serial.begin(115200);
  delay(200);
  DBG("=== Ikoka Nano Winch Remote (XIAO nRF52840 Plus + E22-900M30S) ===");
  ...
  // E22 power-up. The module's 5 V boost is enabled by the reset line, so reset
  // has to be released and the rail allowed to settle before the first SPI
  // transaction. RXEN is parked LOW at the same time: RXEN+TXEN both low is the
  // E22's idle state, and RXEN must not float while the module boots.
  ...
  // Every argument except tcxoVoltage is RadioLib's own default and is re-applied
  ...
  if (loraErr == RADIOLIB_ERR_NONE) {
    ...
    lora.setRfSwitchPins(LORA_RXEN_PIN, RADIOLIB_NC);  // D7 HIGH=RX, LOW=TX
                                                       // (E22 TXEN is driven by the module's own DIO2)
